...
本章介绍了Spring的IOC（控制反转）容器。



[[beans-introduction]]
== 关于Spring IOC和Bean的简介
本章介绍了Spring IOC容器的实现机制 (什么是 <<overview.adoc#background-ioc,控制反转>>.) IoC(控制反转)也被称为依赖注入(DI). 这是一个对象仅仅通过构造函数的参数以及工厂的参数，或者通过对于对象实例在其被构造或者从一个工厂返回后进行的设置，从而定义依赖（是指其他的对象，本对象运行时需要的）的过程。在bean建立之后容器就会注入这些依赖. This process is fundamentally
